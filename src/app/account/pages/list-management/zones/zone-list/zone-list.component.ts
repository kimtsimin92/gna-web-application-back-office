...
export class ZoneListComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  onGetDataList() {
    ...
    this.accountService.getZoneListData(page)
  ...
  onConfirm(data: any): void {
    ...
    const dialogRef = this._dialog.open(ConfirmationRemoveDialogComponent, {
      ...
      height: '340px',
      data: {
        dialogMessage: "du territoire " + data.name
      },
    });
  ...
  private onSave(data: any) {
    ...
    this.accountService.saveZoneRemove(data.id)
  ...
  openSaveNotificationDialog(): void {
    ...
    const dialogRef = this._dialog.open(SaveNotificationDialogComponent, {
      ...
      width: '440px',
      data: {
        dialogMessage: "La suppression du territoire a réussi."
      },
    });
  ...
  openSaveErrorNotificationDialog(error: HttpErrorResponse): void {
    ...
    const dialogRef = this._dialog.open(SaveErrorNotificationDialogComponent, {
      ...
      data: {
        httpError: error,
        dialogMessage: "La suppression du territoire a échoué."
      },
    });
  ...
  onConfirmToggleEnabled(data: any, isToggle: boolean): void {
    ...
    const dialogRef = this._dialog.open(ConfirmationToggleEnableDialogComponent, {
      ...
      height: '340px',
      data: {
        dialogMessage: "le territoire " + data.name,
        isToggle: isToggle
      },
    });
  ...
  private onSaveToggleEnable(data: any) {
    ...
    this.accountService.saveZoneToggleEnable(data.id)
      .subscribe((responseData) => {
        this.isSave = false;
        this.closeDialog();
        this.onGetDataList();
