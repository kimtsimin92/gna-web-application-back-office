...
@Component({
  ...
  styleUrl: './capital-list.component.css'
})
export class CapitalListComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  ngOnInit(): void {
    ...
    if (localStorage.getItem("CAPITAL_DATA")) {
      localStorage.removeItem("CAPITAL_DATA");
    }
  ...
  onAdd() {
    this._router.navigateByUrl("/account/management/products/capitals/add");
  }
  ...
  onGetDataList() {
    ...
    this.accountService.getCapitals(this.pageSort, this.pageOrder, this.pageNumber, this.pageSize)
