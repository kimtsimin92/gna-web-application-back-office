...
@Component({
  selector: 'app-form-quotation-edit',
  standalone: true,
  imports: [
    DropdownModule,
    ImageModule,
...
export class FormQuotationEditComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  ngOnInit(): void {
    ...
    if (localStorage.getItem("FORM_QUOTATION_DATA")) {
      ...
      // @ts-ignore
      this.formQuotationData = JSON.parse(localStorage.getItem("FORM_QUOTATION_DATA"));
      ...
      if (this.formQuotationData) {
        ...
        if (this.formQuotationData.productGroup) {
          this.quotationForm.productGroup.setValue(this.formQuotationData.productGroup.name);
        }
  ...
  onSave() {
    ...
    if (this.formQuotation.value.productGroup) {
      if (this.productGroupList && this.productGroupList.length > 0) {
        this.productGroupList.forEach((g: any) => {
          if (g.name === this.formQuotation.value.productGroup) {
            this.quotationFormData.productGroupId = g.id;
          }
        });
      }
    }
    ...
    if (this.formStepList && this.formStepList.length > 0) {
      ...
      if (this.quotationFormData && this.quotationFormData.steps && this.quotationFormData.steps.length > 0) {
        ...
        this.accountService.editFormQuotation(id, this.quotationFormData)
